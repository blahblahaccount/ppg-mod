...
ModAPI.Register(
    new Modification()
    {
        ...
        NameOverride = "Invisible Human -MorePeople", //new item name with a suffix to assure it is globally unique
        DescriptionOverride = "You can't see him", //new item description
        CategoryOverride = ModAPI.FindCategory("Entities"), //new item category
        ThumbnailOverride = ModAPI.LoadSprite("transparent.png"), //new item thumbnail (relative path)
        AfterSpawn = (Instance) => //all code in the AfterSpawn delegate will be executed when the item is spawned
        {
            //load textures for each layer (see Human textures folder in this repository)
            var skin = ModAPI.LoadTexture("transparent.png");
            var flesh = ModAPI.LoadTexture("transparent.png");
            var bone = ModAPI.LoadTexture("transparent.png");
